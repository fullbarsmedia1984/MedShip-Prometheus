...
function offerUomPayload(input: Omit<HerculesOfferUomRecord, 'id'>) {
  return {
    ...
    raw_per_text: input.rawPerText,
    parsed_per_quantity: input.parsedPerQuantity,
    parsed_per_uom: input.parsedPerUom,
...
function toOfferUom(row: DbRow): HerculesOfferUomRecord {
  return {
    ...
    rawPerText: (row.raw_per_text as string | null) ?? null,
    parsedPerQuantity: toNumber(row.parsed_per_quantity),
    parsedPerUom: (row.parsed_per_uom as string | null) ?? null,
    ...
    volumeUom: (row.volume_uom as string | null) ?? null,
