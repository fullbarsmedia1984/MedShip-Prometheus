...
/** Page size the table requests — the server page must fetch the same. */
export const INVENTORY_PAGE_SIZE = 20

/**
 * First-paint payload computed by the server page with the client's default
 * filter state. Shape mirrors the /api/dashboard/inventory response.
 */
...
export function InventoryPageClient({ initialData }: { initialData: InventoryInitialData | null }) {
  ...
  const [result, setResult] = useState<PaginatedResult<Product> | null>(initialData?.result ?? null)
  const [loading, setLoading] = useState(!initialData)
  // The server page already fetched the default-filter page; skip the first
  // client fetch so first paint doesn't immediately re-request the same data.
  // (Initial filter state above is always the default — there is no URL-param
  // seeding on this page — so the server payload matches it.)
