...
interface OrdersPageClientProps {
  // Server-rendered payload for the DEFAULT filter state. When present, the
  // first paint uses it and the mount fetch is skipped; every subsequent
  // filter/search/pagination change goes through the normal client fetch.
  initialData?: OrdersDashboardResponse
}
...
export function OrdersPageClient({ initialData }: OrdersPageClientProps) {
  ...
  const [result, setResult] = useState<PaginatedResult<Order> | null>(initialData?.result ?? null)
  const [summary, setSummary] = useState<OrderSummary | null>(initialData?.summary ?? null)
  const [dataQuality, setDataQuality] = useState<DataQualitySummary | null>(initialData?.dataQuality ?? null)
  const [salesOrderCoverage, setSalesOrderCoverage] = useState<SalesOrderCoverage | null>(
    initialData?.salesOrderCoverage ?? null
  )
  const [salesReps, setSalesReps] = useState<SalesRep[]>(initialData?.salesReps ?? [])
  const [loading, setLoading] = useState(!initialData)
  const [expandedRow, setExpandedRow] = useState<string | null>(null)
  // The filter state above starts at the exact defaults the server payload
  // was built for, so when initialData is present the first fetch would be a
  // duplicate of what is already rendered — skip it once.
