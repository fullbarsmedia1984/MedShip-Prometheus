...
/**
 * First-paint payload computed by the server page with the client's default
 * filter state. Shape mirrors the /api/hercules/catalog response.
 */
export type CatalogInitialData = CatalogResponse
...
/** Page size the results list requests — the server page must fetch the same. */
  ...
  return (
    <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border bg-white">
      {item.imageUrl && !broken ? (
        isSupabaseHosted(item.imageUrl) ? (
          // next/image resizes the full-resolution mirror down to a real
          // 64px thumbnail (lazy by default) instead of shipping megapixels.
          <Image
            src={item.imageUrl}
            alt=""
            width={64}
...
export function CatalogPageClient({ initialData }: { initialData: CatalogInitialData | null }) {
  const [filters, setFilters] = useState<Filters>(DEFAULT_FILTERS)
  const [debouncedSearch, setDebouncedSearch] = useState('')
  const [result, setResult] = useState<CatalogSearchResult | null>(initialData?.result ?? null)
  const [facets, setFacets] = useState<CatalogFacets | null>(initialData?.facets ?? null)
  const [canSeePrices, setCanSeePrices] = useState(initialData?.canSeePrices ?? false)
  const [loading, setLoading] = useState(!initialData)
  // The server page already ran the default (no search, no filters, page 1)
  // query through the catalog cache; skip the first client fetch so first
  // paint doesn't immediately re-request the same data. Initial filter state
  // is always DEFAULT_FILTERS — this page does not seed from URL params.
  ...
  const from = (filters.page - 1) * CATALOG_PAGE_SIZE + 1
